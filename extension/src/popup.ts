...
import { StyaAPI } from './lib/api';
import type { X10Collection } from './lib/types';
...
interface Elements {
  ...
  videoInfo: HTMLElement | null;
  videoThumbnail: HTMLElement | null;
  videoTitle: HTMLElement | null;
  videoMeta: HTMLElement | null;
  pageInfo: HTMLElement | null;
  pageIcon: HTMLElement | null;
  pageTitle: HTMLElement | null;
  pageMeta: HTMLElement | null;
  notSupported: HTMLElement | null;
  quickActions: HTMLElement | null;
  openDirectBtn: HTMLButtonElement | null;
  openDirectLabel: HTMLElement | null;
  openInBtn: HTMLButtonElement | null;
  llmSubmenu: HTMLElement | null;
  copyLinkBtn: HTMLButtonElement | null;
  copyContentBtn: HTMLButtonElement | null;
  x10sSection: HTMLElement | null;
  x10sList: HTMLElement | null;
  emptyState: HTMLElement | null;
  createBtn: HTMLButtonElement | null;
  dashboardLink: HTMLAnchorElement | null;
  syncLink: HTMLAnchorElement | null;
  logoLink: HTMLAnchorElement | null;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  elements.openDirectBtn = document.getElementById('open-direct-btn') as HTMLButtonElement;
  elements.openDirectLabel = document.getElementById('open-direct-label');
  elements.openInBtn = document.getElementById('open-in-btn') as HTMLButtonElement;
  elements.llmSubmenu = document.getElementById('llm-submenu');
  elements.copyLinkBtn = document.getElementById('copy-link-btn') as HTMLButtonElement;
  elements.copyContentBtn = document.getElementById('copy-content-btn') as HTMLButtonElement;
  elements.x10sSection = document.getElementById('x10s-section');
  elements.x10sList = document.getElementById('x10s-list');
  elements.emptyState = document.getElementById('empty-state');
  elements.createBtn = document.getElementById('create-btn') as HTMLButtonElement;
  elements.dashboardLink = document.getElementById('dashboard-link') as HTMLAnchorElement;
  elements.syncLink = document.getElementById('sync-link') as HTMLAnchorElement;
  elements.logoLink = document.getElementById('logo-link') as HTMLAnchorElement;
  ...
  if (elements.openInBtn && elements.llmSubmenu) {
...
function showNotSupported(): void {
  ...
  if (btnText) {
    btnText.textContent = 'Open a web page first';
  }
}
...
function showVideoInfo(): void {
  ...
  if (currentItem && currentItem.id && elements.videoThumbnail) {
    // Set thumbnail
  ...
  const btnText = document.getElementById('create-btn-text');
  if (btnText) {
    btnText.textContent = 'A new collection';
  }
}

function showPageInfo(): void {
  elements.loading?.classList.add('hidden');
  elements.notSupported?.classList.add('hidden');
  elements.videoInfo?.classList.add('hidden');
  ...
  if (elements.pageTitle && currentItem) {
    elements.pageTitle.textContent = currentItem.title || 'Untitled page';
  }
  if (elements.pageMeta && currentItem) {
...
async function loadX10sList(): Promise<void> {
  ...
  if (result.x10s.length === 0) {
    ...
    if (currentItem && currentItem.type === 'youtube' && currentItem.id) {
...
function renderX10sList(x10s: X10Collection[]): void {
  if (!elements.x10sList) return;
...
async function handleCreateX10(): Promise<void> {
  if (!currentItem) return;
...
async function handleAddToX10(x10Id: string, x10Title: string): Promise<void> {
  if (!currentItem) return;

  // Find and disable the item
  const item = elements.x10sList?.querySelector(`[data-x10-id="${x10Id}"]`) as HTMLElement | null;
  if (item) {
...
function updateDirectButton(llmKey: string): void {
  if (llmKey && LLM_NAMES[llmKey]) {
    if (elements.openDirectLabel) {
...
function disableQuickActions(): void {
  if (elements.openDirectBtn) elements.openDirectBtn.disabled = true;
  if (elements.openInBtn) elements.openInBtn.disabled = true;
  if (elements.copyLinkBtn) elements.copyLinkBtn.disabled = true;
  if (elements.copyContentBtn) elements.copyContentBtn.disabled = true;
}

function enableQuickActions(): void {
  if (elements.openDirectBtn) elements.openDirectBtn.disabled = false;
  if (elements.openInBtn) elements.openInBtn.disabled = false;
  if (elements.copyLinkBtn) elements.copyLinkBtn.disabled = false;
  if (elements.copyContentBtn) elements.copyContentBtn.disabled = false;
}

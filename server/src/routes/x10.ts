...
// CORS middleware for .txt endpoints (needed for extension "Copy Content")
x10Router.use('/*.txt', (req: Request, res: Response, next) => {
...
function buildCollectionMarkdown(collection: CollectionWithItems, collectionId: string): string {
  ...
  collection.items.forEach((item, index) => {
    ...
    md += `**URL**: ${item.url}  \n`;
    md += `**TXT**: ${baseUrl}/s/${collectionId}/v/${itemId}.txt\n\n`;
    md += `${item.transcript}\n\n`;
    md += `---\n\n`;
  });
...
// Redirect .md to .txt for backwards compatibility
x10Router.get('/:id.md', (req: Request, res: Response) => {
  res.redirect(301, `/s/${req.params.id}.txt`);
});

// Collection page (Plain text for LLM) - Primary endpoint
...
// Redirect single item .md to .txt for backwards compatibility
x10Router.get('/:id/v/:itemId.md', (req: Request, res: Response) => {
  res.redirect(301, `/s/${req.params.id}/v/${req.params.itemId}.txt`);
});

// Single item TXT - /s/:id/v/:itemId.txt (itemId can be youtube_id or item.id)
x10Router.get('/:id/v/:itemId.txt', asyncHandler(async (req: Request, res: Response) => {
  ...
  // Use collection's pre_prompt or the default
  const defaultPrePrompt = getDefaultPrePromptText();
  const effectivePrePrompt = collection.pre_prompt || defaultPrePrompt;
  ...
  // Pre-prompt
  if (effectivePrePrompt) {
    content += `${effectivePrePrompt}\n\n`;
  }

  content += `# ${item.title}\n\n`;
  content += `**Type**: ${isYouTube ? 'YouTube Video' : 'Web Page'}  \n`;
  content += `**Source**: ${item.channel}  \n`;
  if (item.duration) {
    content += `**Duration**: ${item.duration}  \n`;
  }
  content += `**URL**: ${item.url}\n\n`;
  content += `---\n\n`;
  content += `## ${isYouTube ? 'Transcript' : 'Content'}\n\n`;
  ...
  content += `*Generated by [${config.brandName}](${config.baseUrl}) — ${date}*\n`;
